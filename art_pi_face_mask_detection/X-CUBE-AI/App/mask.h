...
#ifndef __AI_MASK_H__
#define __AI_MASK_H__
#pragma once

#include "ai_platform.h"
#include "ai_platform_interface.h"

#define AI_MASK_MODEL_NAME          "mask"

#define AI_MASK_ACTIVATIONS_ALIGNMENT   (4)

#define AI_MASK_IN_NUM       (1)
#define AI_MASK_IN { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_U8, 128, 128, 3, 1, NULL), \
}
#define AI_MASK_IN_SIZE { \
  (128 * 128 * 3), \
}
#define AI_MASK_IN_1_SIZE  (128 * 128 * 3)
#define AI_MASK_IN_1_SIZE_BYTES  ((128 * 128 * 3) * 1)





#define AI_MASK_OUT_NUM      (1)
#define AI_MASK_OUT { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_U8, 1, 1, 2, 1, NULL), \
}
#define AI_MASK_OUT_SIZE { \
  (1 * 1 * 2), \
}
#define AI_MASK_OUT_1_SIZE  (1 * 1 * 2)
#define AI_MASK_OUT_1_SIZE_BYTES  ((1 * 1 * 2) * 1)

#define AI_MASK_N_NODES (7)
 ...
 * @ref ai_mask_create. Both the activations memory buffer 
 ...
 * header generated by the code generation tool. See AI_MASK_*
 * defines into file @ref mask.h for all network sizes defines
 *
 * @param network an opaque handle to the network context
